...
impl<T: Peripheral> HardwareConnector for BtleplugHardwareConnector<T> {
  ...
  async fn connect(&mut self) -> Result<Box<dyn HardwareSpecializer>, ButtplugDeviceError> {
    ...
    // VibeLoop patch: always (re)discover services, even when the OS still
    // holds a connection from a previous app run. Upstream skips discovery
    // for already-connected devices, but on BlueZ that yields an empty or
    // partial service table (ServicesResolved can still be pending), and
    // the device fails with InvalidEndpoint("rx") until the stale link
